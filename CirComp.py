...
class Component:
    ...
    def netList(self):
        ...
        if self.kind == "Diode":
            return self.kind[0] + output + DIODE_MODEL

        if self.kind == "Vsource":
            return self.kind[0] + output + " dc " + str(self.params[0])

        if self.kind in ["Resistor", "Capacitor", "Inductor"]:
            return self.kind[0] + output +  " " + str(self.params[0])
        
        output += " " + self.nodes[2]

        if self.kind == "NPN":
            return  "Q" + output + " " + NPN_MODEL
        if self.kind == "PNP":
            return  "Q" + output + " " + PNP_MODEL
        ...
        if self.kind == "NMOS":
            ...
            return "M" + output + " " +  self.nodes[2] + " NMOS W=" + str(self.params[0]) + " L=" + str(self.params[1])

        return "M" + output + " " + self.nodes[1] +" PMOS W=" + str(self.params[0]) + " L=" + str(self.params[1])
